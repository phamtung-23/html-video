...
export function buildCaptionAss(
  ...
  for (const cue of cues) {
    ...
    for (let i = 0; i < words.length; i++) {
      ...
      events.push(
        `Dialogue: 0,${assTime(start)},${assTime(end)},CapGlow,,0,0,0,,{\\blur${glowBlur}}${shown}`,
      );
      events.push(
        `Dialogue: 1,${assTime(start)},${assTime(end)},Cap,,0,0,0,,${shown}`,
      );
    }
  }
