...
async function main() {
  ...
  // 2. Add assets
  log("add image asset");
  let p = await ctx.orchestrator.addFileAsset(
    project1.id,
    fakeLogoPath,
    "OD logo",
  );
  ...
  p = await ctx.orchestrator.addInlineAsset(
    project1.id,
    "Design that evolves itself",
    "text",
  );
  ok(`assets=${p.assets.length}`);
  ...
  ok(
    `templateId=${p.templateId} variables(after-defaults)=${JSON.stringify(p.variables).slice(0, 80)}…`,
  );
  ...
  const { project: previewedProj, htmlPath } =
    await ctx.orchestrator.renderPreviewHtml(project1.id);
  ...
  for (const node of graph.nodes) {
    ...
    const { frame } = await ctx.orchestrator.writeFrameHtml(
      project2.id,
      node.id,
      html,
    );
    ok(
      `frame written: ${frame.graphNodeId} order=${frame.order} dur=${frame.durationSec}s path=${frame.htmlPath.split("/").slice(-3).join("/")}`,
    );
  }
