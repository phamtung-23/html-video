...
function version(cmd: string, args = "--version"): string | null {
  try {
    return (
      execSync(`${cmd} ${args}`, { stdio: ["ignore", "pipe", "ignore"] })
        .toString()
        .trim()
        .split("\n")[0] ?? null
    );
...
export async function runDoctor(ctx: CliContext): Promise<void> {
  ...
  checks.push({
    name: "node-version",
    status: Number.parseInt(nodeV.slice(1)) >= 20 ? "ok" : "warning",
    value: nodeV,
  ...
  if (which("ffmpeg")) {
    checks.push({
      name: "ffmpeg",
      status: "ok",
      value: version("ffmpeg", "-version")?.split(" ")[2] ?? "?",
    });
  } else {
    checks.push({
      name: "ffmpeg",
  ...
  checks.push({
    ...
    detail: chromiumOk
      ? "Chrome found in standard location"
      : "Chrome/Chromium not detected; HF render will need a browser",
  });
  ...
  checks.push({
    ...
    detail:
      tcount === 0
        ? "No templates found in templates/ — install or scaffold some"
        : undefined,
  });

  // edge-tts (free, key-less narration). Optional: a warning, never an error.
  const tts = ctx.mediaConfig.getTtsStatus();
  if (tts.edgeAvailable) {
    checks.push({
      name: "edge-tts",
      status: "ok",
      value: tts.edgeVoice,
    ...
    checks.push({
      ...
      status: "warning",
      detail:
        "Narration disabled. Install free Edge-TTS: pipx install edge-tts",
      install_hint:
        "pipx install edge-tts  (or: python3 -m pip install edge-tts)",
    });
  }
