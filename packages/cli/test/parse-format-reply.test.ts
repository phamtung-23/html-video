...
test('the exact issue #2 reply: "16:9 横屏 / 5s / 10"', () => {
  ...
  assert.equal(r?.aspect, "16:9 Ngang");
...
test('chinese commas + 秒/帧 units: "9:16 竖屏，3秒，6帧"', () => {
  ...
  assert.equal(r?.aspect, "9:16 Dọc");
...
test('keyword-only aspect, no ratio: "方形 5s"', () => {
  ...
  assert.equal(r?.aspect, "1:1 Vuông");
  assert.equal(r?.duration, "5");
});

test("xiaohongshu keyword maps to 4:5", () => {
  assert.equal(parseFormatReply("小红书 10秒 8帧")?.aspect, "4:5 RedNote");
});

test("partial: just an aspect still counts", () => {
  assert.deepEqual(parseFormatReply("横屏"), { aspect: "16:9 Ngang" });
});
...
test("long prose is content, not a format answer", () => {
  assert.equal(
    parseFormatReply(
      "我想做一个介绍我们公司新产品发布的视频，重点讲三个核心卖点和定价",
    ),
    undefined,
  );
});
